...
def getLottoData(start, lastLottoNo) :
    ...
    for i in range(start, lastLottoNo+1):        
        ...
        # github에 업로드
        token = "b4b7f1741cac4f3d74297321583a920a51eea9b6"
        repo = "ptwspirit/lotto_info"        

        g = Github(token)
        data = json.dumps(lotto_dict, ensure_ascii=False)

        repository  = g.get_repo(repo)
        try: 
            contents = repository.get_contents("round_json_" + round)
            repository.update_file("round_json_" + round, "commit", data, contents.sha, branch="master")            
        except:
            repository.create_file("round_json_" + round, "commit", data, branch="master")
...
if __name__ == '__main__':
    ...
    if len(df) == counter:
        ...
        nowDatetime = now.strftime('%Y-%m-%d %a %H:%M:%S')
        commitString = str(lastLottoRound) + " " + nowDatetime

        try: 
            logContents = repository.get_contents("log")        
            print(nowDatetime)
            repository.update_file("log", "commit", commitString, logContents.sha, branch="master")                
        except:
            print(nowDatetime)
            repository.create_file("log", "commit", commitString, branch="master")    
