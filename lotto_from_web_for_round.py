...
def getLottoData(start, lastLottoNo) :
    ...
    for i in range(start, lastLottoNo+1):        
        ...
        # github에 업로
        token = "a87a6e2cde97b55433a04b7ebce3583ada464b1c"
        repo = "ptwspirit/lotto_info"
        path = "round_json_" + round
        ...
        try: 
            contents = repo.get_contents("round_json_" + round)
            repo.update_file("round_json_" + round, "commit", data, contents.sha, branch="master")            
        except:
            repo.create_file("round_json_" + round, "commit", data, branch="master")            
        
    return df
        
if __name__ == '__main__':
    lastLottoRound = getLastLottoRound()
    start = lastLottoRound - 2
